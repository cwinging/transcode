...
class FFMpeg(object):
    ...
    @staticmethod
    def _spawn(cmds):
        logger.debug('Spawning ffmpeg with command: ' + FFMpeg.utf8_str_join(cmds))
        print 'Spawning ffmpeg with command: ' + FFMpeg.utf8_str_join(cmds)
    ...
    def convert(self, infile, outfile, opts, timeout=20):
        """
        ...
        while True:
            ...
            if '\r' in buf:
                ...
                try:
                    ...
                    print 'convert 3 infile=', infile, ',error=', e
                if len(tmp) == 1:
                    timespec = tmp[0]
                    if ':' in timespec:
                        timecode = 0
        ...
        cmd = self.utf8_str_join(cmds)
        #cmd = ' '.join(cmds)
        print 'ffmpeg output=%s' % total_output
        if '\n' in total_output:
    ...
    def concat(self, outfile, format, infiles, timeout=20):
        """
        concat file1 file2 file3 .... to output_file
        @param outfile: concat target file name
        @param format: file format, example mp4, flv
        @param infiles: a list of tuples like: [file1, file2, file3,...]

        >>> FFMpeg.concat('test.flv', 'flv', [test_1.flv, test_2.flv, test_3.flv,])
        >>>
        """
        if len(infiles) == 0:
            raise FFMpegError('Error concat: param infiles is empty')

        cmds = [self.ffmpeg_path]
        concat_holdspace = ''
        for i, fname in enumerate(infiles):
            cmds.extend(['-i', fname])
            av_streams = '%s %s' % ('[%d:v]' % i, '[%d:a:0]' % i)
            concat_holdspace += av_streams

        concat_filter_cmds = '%s concat=n=%d:v=1:a=1 [v] [a]' % (concat_holdspace, len(input_files))
        cmds.extend(['-filter_complex', concat_filter_cmds])
        cmds.extend(['-map', 'v'])
        cmds.extend(['-map', 'a'])
        cmds.extend(['-f', format])
        cmds.extend(['-y', outfile])
        ...
        if '\n' in total_output:
            ...
            if line.startswith('Error while '):
                raise FFMpegConvertError('Encoding error', cmd, total_output,
                                         line, pid=p.pid)
            if line.startswith('Invalid argument'):
                raise FFMpegConvertError('concat filter params invalid', cmd, total_output,
                                         line, pid=p.pid)
            if line.startswith('Error configuring'):
                raise FFMpegConvertError('Error configuring complex filters', cmd, total_output,
                                         line, pid=p.pid)
            if not yielded:
                raise FFMpegConvertError('Unknown ffmpeg error', cmd,
                                         total_output, line, pid=p.pid)
