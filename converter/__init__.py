...
class Converter(object):
    ...
    def convert(self, infile, outfile, options, twopass=False, timeout=20):
        """
        ...
        if twopass:
            ...
            for timecode in self.ffmpeg.convert(infile, outfile, optlist1,
                                                timeout=timeout):
                yield (int((50.0 * timecode) / info.format.duration), timecode)

            optlist2 = self.parse_options(options, 2)
            for timecode in self.ffmpeg.convert(infile, outfile, optlist2,
                                                timeout=timeout):
                yield (int(50.0 + (50.0 * timecode) / info.format.duration), timecode)
        else:
            optlist = self.parse_options(options, twopass)
            for timecode in self.ffmpeg.convert(infile, outfile, optlist,
                                                timeout=timeout):
                yield (int((100.0 * timecode) / info.format.duration), timecode)
    ...
    def concat(self, outfile, format, infiles, timeout=20):
        """
        """
        return self.ffmpeg.concat(outfile, format, infiles, timeout)
